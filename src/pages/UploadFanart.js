        ...
        return (
          <div className="md:px-8">
            <label
              ...
              className="w-full aspect-[9/16] md:aspect-video border-4 border-dashed flex flex-col justify-center items-center"
            >
      ...
      case 1:
        return (
          <div>
            <div
              onClick={() =>
                setShownImages([{ src: imagePreview[fanartIndex], alt: "" }])
              }
              className="flex items-center justify-center min-h-[350px] md:min-h-[480px] cursor-zoom-in relative"
            >
              ...
              {imagePreview.length > 1 && (
                <div
                  style={{
                    ...
                    maxWidth: 80 * (isMobile ? 3 : 5),
                  }}
                  className="h-20 absolute bg-black/50 left-1/2 bottom-0 cursor-pointer"
                >
                  <div className="relative h-full">
                    <div
                      onClick={(e) => clickMenu(e)}
                      className="absolute h-full flex items-center hover:bg-black/50 bg-black/25 transition left-0 top-0"
                    >
                      <FontAwesomeIcon icon="chevron-left" className="mx-2" />
                    </div>
                    <div
                      onClick={(e) => clickMenu(e, true)}
                      className="absolute h-full flex items-center hover:bg-black/50 bg-black/25 transition right-0 top-0"
                    >
                      <FontAwesomeIcon icon="chevron-right" className="mx-2" />
                    </div>
                    <div
                      ref={imageMenu}
                      className="h-full flex overflow-x-scroll"
                    >
                      {imagePreview.map((file, i) => (
                        <div
                          className="aspect-square p-0.5 flex items-center justify-center h-full"
                          onClick={(e) => {
                            e.stopPropagation();
                            setFanartIndex(i);
                          }}
            ...
            <div className="flex mt-8 pt-4 border-t border-slate-700 flex-wrap w-full">
              <div className="w-full md:w-1/2">
                <label>
                  ...
                  <input
                    type="text"
                    value={fanartForm.title}
                    onChange={(e) => setImage("title", e.target.value)}
                ...
                <label>
                  <div>Description</div>
                  <textarea
                    value={fanartForm.desc}
                    onChange={(e) => setImage("desc", e.target.value)}
                    className="focus-visible:outline-none focus-visible:bg-[#13202c] py-1 px-2 bg-[#00202b] rounded w-full"
                  ></textarea>
                </label>
              </div>
              <div className="w-full md:w-1/2 md:pl-2">
                <label>
                  <div>Tags</div>
                  <textarea
                    value={fanartForm.tags}
                    onChange={(e) => setImage("tags", e.target.value)}
                    className="focus-visible:outline-none focus-visible:bg-[#13202c] py-1 px-2 bg-[#00202b] rounded w-full"
                  ></textarea>
                </label>
              </div>
