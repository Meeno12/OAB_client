...
export default function UploadMusic() {
  ...
  const [musicForm, setMusicForm] = useState({
    ...
    childs: new Array(4).fill({
      title: "",
      tags: "",
      links: [{ platform: "youtube", url: "", valid: false }],
    }),
  });
  ...
  const setMusic = (attr, val) =>
    setMusicForm({
      ...musicForm,
      childs: musicForm.childs.map((x, i2) =>
        songIndex === i2 ? { ...music, [attr]: val } : x
      ),
    });

  const setLink = (index, attr, val) =>
    setMusic(
      "links",
      music.links.map((x, i) => (i === index ? { ...x, [attr]: val } : x))
    );
  ...
  const setUrl = (e, i) => {
    ...
    if (
      music.links
        .map((x, idx) => (idx === i ? valid : x.valid))
        .every((link) => link)
    ) {
      const availablePlatform = musicPlatform.filter(
        (platform) => !music.links.find((x) => platform === x.platform)
      );
      if (!availablePlatform[0]) {
        setMusic(
          "links",
          music.links.map((x, idx) =>
            idx === i ? { ...x, url: val, valid } : x
          )
        );
        return;
      ...
      jsmediatags.read(file, {
        onSuccess: (data) => {
          ...
          if (!hasCover && data.tags.picture) {
            ...
            setAlbumCover(
              `data:${picture.format};base64,${Buffer.from(
                picture.data
              ).toString("base64")}`
            );
            hasCover = true;
          ...
          musics.push({
            title: data.tags.title || file.name,
            tags: "",
            links: [{ platform: "youtube", url: "", valid: false }],
          });
        ...
        return (
          <div className="md:px-8">
            <label
              ...
              className="tooltip w-full aspect-[9/16] md:aspect-video border-4 border-dashed flex flex-col justify-center items-center"
            >
              <div className="text-center text-slate-400 relative">
                ...
                <div style={{ width: 160 }} className="tooltip-text">
            ...
            {files[0] && (
              <div className="flex justify-between mt-4">
                <div></div>
                <Button variant="solid" onClick={() => setPage(1)}>
                  Next
                </Button>
              </div>
            )}
          ...
          <div className="flex flex-wrap flex-row-reverse">
            <div className="p-2 w-full md:w-[30%] flex items-center flex-col">
              <label className="flex justify-center">
                <img
                  src={albumCover ? albumCover : ""}
                  className="w-11/12 aspect-square object-cover"
                />
                  ...
                  <tbody>
                    {musicForm.childs.map((x, i) => (
                      <tr
                        ...
                        key={i}
                        className={
                          "hover:bg-white/[.05] cursor-pointer" +
                          (songIndex === i ? " bg-white/[.05]" : "")
                        }
                      >
                        ...
                        <td className="text-right pr-7 py-1.5">
                          <FontAwesomeIcon
                            icon={x.title ? "check" : "circle-exclamation"}
                          />
                        </td>
                      </tr>
                    ))}
            ...
            <div className="p-2 w-full md:w-[70%]">
              ...
              <label>
                ...
                <textarea
                  value={music.tags}
                  onChange={(e) => setMusic("tags", e.target.value)}
                  className="focus-visible:outline-none focus-visible:bg-[#13202c] py-1 px-2 bg-[#00202b] rounded w-full"
                ></textarea>
              </label>
              <div>
                ...
                {music.links.map((link, i) => (
                  <div key={i} className="flex flex-wrap mb-0.5">
                    <div
                      onMouseLeave={() => setLinkDrodown(false)}
                      onClick={() =>
                        !link.valid &&
                        setLinkDrodown(linkDrodown === i ? false : i)
                      }
                      className={
                        (!link.valid ? "hover:bg-[#13202c] " : "") +
                        "bg-[#00202b] relative cursor-pointer border-r flex items-center px-2 border-slate-700"
                      }
                    >
                      <div>
                        <FontAwesomeIcon
                      ...
                      <div
                        style={{ zIndex: 5 }}
                        className={
                          "absolute top-full left-0 w-full pt-1" +
                          (linkDrodown === i ? "" : " hidden")
                        }
                      >
                        {musicPlatform
                          .filter(
                            (platform) =>
                              !music.links.find((x) => platform === x.platform)
                          )
                          .map((x, i2) => (
                            <div
                              onClick={() => setLink(i, "platform", x)}
                              key={i2}
                              className="bg-[#00202b] hover:bg-[#13202c] p-1 px-2 flex justify-center"
                            >
                              <FontAwesomeIcon
                                icon={"fab fa-" + x}
                                className="w-6 h-6"
            ...
            <div className="w-full flex mt-4 justify-between">
              <Button variant="solid" onClick={() => setPage(0)}>
                Prev
              </Button>
  ...
  return (
    <div className="bg-[#001820] min-h-screen">
      <div className="mx-auto pt-20" style={{ maxWidth: 1150 }}>
        <div className="bg-[#002e3f] text-slate-50 p-5">{currentPage()}</div>
      </div>
    </div>
  );
}
